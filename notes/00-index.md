...
| 地址 | 角色 | 用途 |
|------|------|------|
| `0xddddddddddddddddddddddddddddddddddddddd1` | **to** (路由地址) | 发送到此地址的交易路由到 DexVM，用户无需持有此地址私钥 |
| `0x0000000000000000000000000000000000000100` | **to** (预编译地址) | EVM 调用此地址触发跨 VM 操作 |
| 用户自己的地址 | **from** (发送者) | 从签名恢复，作为计数器的 key |

> **注意**: 路由地址 `0xddd...dd1` 只是一个"门牌号"，告诉系统走 DexVM 通道。计数器的 key 是用户自己的地址 (from)，不是路由地址。详见 [02-transaction-flow.md#8-地址角色详解](02-transaction-flow.md)
